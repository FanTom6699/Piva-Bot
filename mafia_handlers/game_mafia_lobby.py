...
# --- ✅ НОВЫЙ ИМПОРТ ФУНКЦИИ ПРОВЕРКИ ---
from .game_mafia_core import MafiaGameStates, distribute_roles_and_start, check_players_availability 
# --- --- ---
...
async def lobby_timer_task(chat_id: int, bot: Bot, db: Database, settings: SettingsManager):
    ...
    if player_count < settings.mafia_min_players:
        ...
        await bot.send_message(chat_id=chat_id, text="⏰ Время вышло, а игроков не набралось. Игра отменена.") # ✅ ИСПРАВЛЕНО
        ...
        # --- ✅ НОВАЯ ПРОВЕРКА НА ДОСТУП В ЛС ---
        players = await db.get_mafia_players(chat_id)
        failed_ids, player_names = await check_players_availability(bot, db, chat_id, players)
        
        if failed_ids:
            logging.warning(f"[Mafia {chat_id}] Таймер лобби: Игроки не запустили бота. Отмена.")
            failed_names = [player_names[uid] for uid in failed_ids]
            await bot.send_message(
                chat_id=chat_id,
                text="🚫 <b>Авто-старт отменен!</b>\n\n"
                     "Следующие игроки должны <b>сначала запустить бота в ЛС</b>:\n"
                     f"• {', '.join(failed_names)}\n\n"
                     "Попросите их это сделать и запустите игру вручную.",
                parse_mode="HTML"
            )
            # Отменяем игру
            await db.delete_mafia_game(chat_id)
            ...
            with suppress(TelegramBadRequest):
                await bot.edit_message_text(
                    text="🚫 Лобби отменено: не все игроки активировали бота в ЛС.",
                    chat_id=chat_id,
        ...
        await distribute_roles_and_start(chat_id, bot, db, settings, players, player_names)
...
async def cmd_mafia_start(message: Message, bot: Bot, db: Database, settings: SettingsManager):
    ...
    with suppress(TelegramBadRequest):
        await bot.pin_chat_message(chat_id=chat_id, message_id=lobby_message.message_id, disable_notification=True)
        ...
        if main_bot_token:
            try:
                ...
                await bot.send_message(
                    ...
                    text=f"Похоже, ты не зарегистрирован в основном боте 'Пивной'.\n"
...
async def handle_start_game_logic(message_or_callback: Message | CallbackQuery, bot: Bot, db: Database, settings: SettingsManager):
    """Общая логика для /startgame и кнопки '▶️ Начать игру'."""
    chat_id = message_or_callback.chat.id
    
    game = await db.get_mafia_game(chat_id)
    if not game or game[3] != 'lobby':
        if isinstance(message_or_callback, CallbackQuery):
            await message_or_callback.answer("Лобби не найдено.", show_alert=True)
        else:
            await message_or_callback.reply("Лобби не найдено.")
        return

    player_count = await db.get_mafia_player_count(chat_id)
    if player_count < settings.mafia_min_players:
        text = f"Недостаточно игроков. (Мин: {settings.mafia_min_players})"
        if isinstance(message_or_callback, CallbackQuery):
    ...
    if failed_ids:
        logging.warning(f"[Mafia {chat_id}] Ручной старт: Игроки не запустили бота. Отмена.")
        failed_names = [player_names[uid] for uid in failed_ids]
        ...
        # Сообщаем об ошибке
        text = ("🚫 <b>Игра не может начаться!</b>\n\n"
                "Следующие игроки должны <b>сначала запустить бота в ЛС</b>:\n"
                f"• {', '.join(failed_names)}\n\n"
                "Лобби отменено. Попросите их активировать бота и начните набор заново.")
                
        if isinstance(message_or_callback, CallbackQuery):
            await message_or_callback.answer("Ошибка! Игроки не запустили бота. (См. в чате)", show_alert=True)
            await bot.send_message(chat_id=chat_id, text=text, parse_mode="HTML")
        else:
            await message_or_callback.reply(text, parse_mode="HTML")
        return
    # --- --- ---
    ...
    text = "Принудительный запуск игры..."
    if isinstance(message_or_callback, CallbackQuery):
        await message_or_callback.answer(text)
    else:
        await message_or_callback.reply(text)
    
    # Все проверки пройдены, передаем player_names, которые мы уже получили
    await distribute_roles_and_start(chat_id, bot, db, settings, players, player_names)
...
async def cq_mafia_start_game(callback: CallbackQuery, bot: Bot, db: Database, settings: SettingsManager):
    """Обработчик кнопки '▶️ Начать игру' (только для Админов)"""
    await handle_start_game_logic(callback, bot, db, settings)
...
async def cmd_mafia_force_start(message: Message, bot: Bot, db: Database, settings: SettingsManager):
    """Обработчик команды /startgame (только для Админов)"""
    await handle_start_game_logic(message, bot, db, settings)
