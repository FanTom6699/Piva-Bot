...
class Database:
    ...
    async def modify_inventory(self, user_id: int, item_id: str, amount: int) -> bool:
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            # Проверяем, прошла ли операция
            # (Если мы пытались списать 10, а было 5, (5 + (-10)) < 0, WHERE не сработает)
    ...
    async def start_upgrade(self, user_id: int, building: str, end_time: datetime, cost: int):
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            if building == 'field':
                # (Получаем уровень прямо перед апгрейдом)
