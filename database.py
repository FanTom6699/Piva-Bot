...
class Database:
    ...
    # --- ✅✅✅ (Piva Bot) ВОТ ОН, ФИКС СПИСАНИЯ СЕМЯН! ✅✅✅ ---
    async def modify_inventory(self, user_id: int, item_id: str, amount: int) -> bool:
        ...
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            ...
            # (Piva Bot: ФИКС БАГА)
            # 'amount' (e.g. -1) передается 3 раза.
            # 1. 'max(0, ?)' для INSERT (если предмета нет, он не станет -1)
            # 2. 'quantity + ?' для UPDATE (здесь мы используем -1)
            # 3. 'WHERE (quantity + ?) >= 0' для UPDATE (проверка)
