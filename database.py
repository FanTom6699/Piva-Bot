...
class Database:
    ...
    async def initialize(self):
        ...
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            ...
            # --- ✅✅✅ ИЗМЕНЕНИЕ ЗДЕСЬ (user_farm_plots) ✅✅✅ ---
            await db.execute('''
            ...
            ''')
            # (Миграция: добавляем колонку, если ее нет)
            try:
    ...
    # --- (Функции Юзеров... add_user изменен) ---
    ...
    # --- (Функции Фермы... _ensure_farm_data, _ensure_inventory, get_user_farm_data, get_user_plots, get_user_inventory, modify_inventory... без изменений) ---
    ...
    async def get_user_farm_data(self, user_id: int) -> Dict[str, Any]:
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            ...
            row = await cursor.fetchone()
            if not row:
                logging.warning(f"Данные фермы для {user_id} не найдены, создаю...")
                await db.commit() 
                cursor = await db.execute("SELECT * FROM user_farm_data WHERE user_id = ?", (user_id,))
                row = await cursor.fetchone()
                if not row:
    ...
    # --- (start_brewing, collect_brewery, start_upgrade, finish_upgrade... без изменений) ---
    ...
    async def get_pending_notifications(self) -> List[Tuple]:
        """(Для farm_updater) Находит ТОЛЬКО апгрейды и варку."""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            now = datetime.now()
    ...
    async def mark_notification_sent(self, user_id: int, task_type: str):
        """(Для farm_updater) Помечает апгрейд/варку как отправленное."""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            await db.execute(
    ...
    # --- ✅✅✅ НОВЫЕ ФУНКЦИИ (Для уведомлений о полях) ✅✅✅ ---
    async def get_pending_crop_notifications(self) -> List[Tuple]:
        """(Для farm_updater) Находит готовые поля, о которых не уведомили."""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            now_iso = datetime.now().isoformat()
            cursor = await db.execute(
    ...
    async def mark_crop_notification_sent(self, user_id: int, plot_num: int):
        """(Для farm_updater) Помечает поле как 'уведомлено'."""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            await db.execute(
