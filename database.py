...
class Database:
    ...
    async def initialize(self):
        ...
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            ...
            ''')
            # --- ✅✅✅ МИГРАЦИЯ ДЛЯ /me (Добавляем колонку, если ее нет) ---
            try:
            ...
            # --- ✅✅✅ НОВЫЕ ТАБЛИЦЫ ДЛЯ ФЕРМЫ (Твой код) ✅✅✅ ---
            await db.execute('''
    ...
    # --- ✅✅✅ ИЗМЕНЕНО (для !напоить) ---
    async def get_user_by_id(self, user_id: int):
        """Ищет по ID, возвращает (Имя, Фамилию) для отображения."""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
    ...
    async def get_user_by_username(self, username: str):
        """Ищет по @username, возвращает (ID, Имя) для !напоить."""
    ...
    async def get_user_reg_date(self, user_id: int) -> str | None:
        """Возвращает дату регистрации (ISO)"""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
    ...
    async def get_user_raid_stats(self, user_id: int) -> Tuple[int, int]:
        """Возвращает (Кол-во рейдов, Суммарный урон)"""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            query = """
    ...
    async def modify_inventory(self, user_id: int, item_id: str, amount: int) -> bool:
        """(✅ ВАЖНО) Изменяет кол-во предмета. Возвращает False, если не хватает."""
        async with aiosqlite.connect(self.db_name, timeout=20) as db:
            await self._ensure_inventory(db, user_id)
