...
class Database:
    ...
    async def get_user_by_id(self, user_id: int):
        async with aiosqlite.connect(self.db_name) as db:
            ...
            if row:
                # (Возвращаем Имя и Юзернейм для /give)
                name = row[0] if row[0] else (row[1] if row[1] else "Игрок")
                return user_id, name
            return None
    ...
    async def get_user_by_username(self, username: str):
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            cursor = await db.execute('SELECT user_id, first_name, last_name FROM users WHERE lower(username) = lower(?)', (username,))
            row = await cursor.fetchone()
            if row:
                name = row[1] if row[1] else (row[2] if row[2] else "Игрок")
                return row[0], name
            return None
    ...
    async def get_user_farm_data(self, user_id: int) -> Dict[str, Any]:
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            row = await cursor.fetchone()
            if not row:
                # (Если юзер был создан до user_farm_data, создаем ее)
                await self._ensure_farm_data(db, user_id)
                await db.commit()
                cursor = await db.execute("SELECT * FROM user_farm_data WHERE user_id = ?", (user_id,))
                row = await cursor.fetchone()
            
            # Конвертируем таймеры из ISO строк в datetime (если они есть)
    ...
    async def get_user_inventory(self, user_id: int) -> Dict[str, int]:
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            # Создаем полный словарь, включая нули
    ...
    async def modify_inventory(self, user_id: int, item_id: str, amount: int) -> bool:
        """Изменяет кол-во предмета. Возвращает False, если не хватает."""
        async with aiosqlite.connect(self.db_name) as db:
            await self._ensure_inventory(db, user_id)
            
            # (UPSERT)
            # Вставляем или Обновляем
            # max(0, ...) гарантирует, что кол-во не станет отрицательным в БД
            await db.execute(
                """
            ...
            # Если мы пытались списать (amount < 0) и ничего не изменилось (changes == 0)
    ...
    async def plant_crop(self, user_id: int, plot_num: int, crop_id: str, ready_time: datetime) -> bool:
        """Сажает урожай на участок. Возвращает False, если участок занят."""
        async with aiosqlite.connect(self.db_name) as db:
            try:
                await db.execute(
    ...
    async def collect_brewery(self, user_id: int, reward: int):
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            await self.change_rating(user_id, reward) # (Используем встроенную функцию)
            await db.commit()
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            if building == 'field':
                ...
                await db.execute(
                    ...
                    (user_id, str(level + 1)) # Сохраняем НОВЫЙ уровень
                )
            else: # brewery
    ...
    async def get_pending_notifications(self) -> List[Tuple]:
        ...
        async with aiosqlite.connect(self.db_name) as db:
            ...
            cursor_field = await db.execute(
                ...
                "WHERE T1.task_type = 'field_upgrade' AND T1.is_sent = 0 AND T2.field_upgrade_timer_end IS NOT NULL AND T2.field_upgrade_timer_end <= ?",
                (now.isoformat(),)
            )
            field_tasks = await cursor_field.fetchall()
            
            # 2. Апгрейды Пивоварни
            cursor_brewery = await db.execute(
                "SELECT T1.user_id, T1.task_type, T1.data_json FROM farm_notifications T1 "
                "JOIN user_farm_data T2 ON T1.user_id = T2.user_id "
                "WHERE T1.task_type = 'brewery_upgrade' AND T1.is_sent = 0 AND T2.brewery_upgrade_timer_end IS NOT NULL AND T2.brewery_upgrade_timer_end <= ?",
                (now.isoformat(),)
            )
            ...
            cursor_batch = await db.execute(
                "SELECT T1.user_id, T1.task_type, T1.data_json FROM farm_notifications T1 "
                "JOIN user_farm_data T2 ON T1.user_id = T2.user_id "
                "WHERE T1.task_type = 'batch' AND T1.is_sent = 0 AND T2.brewery_batch_timer_end IS NOT NULL AND T2.brewery_batch_timer_end <= ?",
                (now.isoformat(),)
            )
            ...
            return [(uid, ttype, int(data)) for uid, ttype, data in all_tasks]
