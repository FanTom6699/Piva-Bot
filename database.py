...
class Database:
    ...
    async def get_user_farm_data(self, user_id: int) -> Dict[str, Any]:
        async with aiosqlite.connect(self.db_name) as db:
            ...
            row = await cursor.fetchone()
            if not row:
                # (Если юзер новый, создаем и запрашиваем снова)
                logging.warning(f"Данные фермы для {user_id} не найдены, создаю...")
                await db.commit() # (Коммитим INSERT из _ensure_farm_data)
                cursor = await db.execute("SELECT * FROM user_farm_data WHERE user_id = ?", (user_id,))
                row = await cursor.fetchone()
                if not row:
                     logging.error(f"КРИТИЧЕСКАЯ ОШИБКА: Не удалось создать farm_data для {user_id}")
                     return {} # (Возвращаем пустой, чтобы избежать сбоя)
    ...
    # ЭТА ФУНКЦИЯ 'plant_crop' — ТО, ЧТО ТЕБЕ НУЖНО
    # (Она чинит "зависшие" кнопки Участка)
    #
    # --- --- --- --- --- --- --- --- --- --- --- --- --- ---
    ...
    async def start_upgrade(self, user_id: int, building: str, end_time: datetime, cost: int):
        ...
        async with aiosqlite.connect(self.db_name) as db:
            await self._ensure_farm_data(db, user_id) # (Убеждаемся, что запись есть)
