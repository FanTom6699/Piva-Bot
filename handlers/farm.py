...
async def get_farm_dashboard(user_id: int, user_name: str, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    batch_timer = farm.get('brewery_batch_timer_end') # (Piva Bot: Возвращаем Таймер)
    ...
    elif batch_timer: # (Piva Bot: Возвращаем Таймер)
        if now >= batch_timer:
    ...
    elif (not batch_timer and not brew_upgrade_timer and # (Piva Bot: Возвращаем 'batch_timer')
    ...
    # Кнопка Пивоварни (Piva Bot: Возвращаем логику Таймера)
    if brew_upgrade_timer and now < brew_upgrade_timer:
        kb.append([InlineKeyboardButton(
...
async def cq_brewery_menu(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    if max_brew > 0:
        ...
        for qty in (1, 5, 10):
            if max_brew >= qty:
                ...
                btns.append(InlineKeyboardButton(
                    text=f"🔥 {qty} ({format_time_delta(total)})", # (Piva Bot: Время снова здесь)
        ...
        if max_brew not in (1, 5, 10) and max_brew > 0:
            ...
            btns.append(InlineKeyboardButton(
                text=f"🔥 MAX {max_brew} ({format_time_delta(total)})", # (Piva Bot: Время снова здесь)
...
async def cq_brewery_do(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    # (Piva Bot: Возвращаем db.start_brewing)
...
# --- ✅✅✅ (Piva Bot) ФИКС "ЗАВИСШЕЙ" КНОПКИ (try...except) ✅✅✅ ---
...
async def cq_brewery_collect(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    try:
        ...
        # (Piva Bot: Это может быть "database is locked")
