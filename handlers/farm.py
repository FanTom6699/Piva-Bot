...
async def cq_plot_plant_do(callback: CallbackQuery, callback_data: PlotCallback, db: Database):
    ...
    # ✅✅✅ ФИКС TypeError (Строка 359) ✅✅✅
    # Принудительно превращаем в ЧИСЛО (int)
    time_m = int(PLANT_IO[crop_id][1]) 
    ...
    try:
        ...
        # (Piva Bot: Убедись, что твой db.plant_crop ждет 4 аргумента)
...
async def cq_brewery_do(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    # (Piva Bot: Также исправляем Пивоварню, на всякий случай)
