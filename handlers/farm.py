...
# --- ✅ НОВЫЕ ИМПОРТЫ (Для Доски Заказов) ---
...
# --- ✅✅✅ БЛОКИРОВКА УДАЛЕНА ✅✅✅ ---
# (block_farm_in_groups удален. Теперь /farm будет работать везде)
# --- ---

# --- FSM (для Магазина) ---
class FarmStates(StatesGroup):
    shop_buy_amount = State() 

# --- FarmCallback (с Доской Заказов) ---
...
async def check_owner(callback: CallbackQuery, callback_data: FarmCallback) -> bool:
    """(Piva Bot) Это и есть "команда работает на того кто вызвал его"."""
...
# --- 1. ГЛАВНОЕ МЕНЮ (/farm) ---
# (Этот хэндлер теперь будет ловить /farm везде)
@farm_router.message(Command("farm"))
async def cmd_farm_start(message: Message, bot: Bot, db: Database):
    # (check_user_registered будет перехватывать не-юзеров в группе)
    ...
    # (Используем .answer() для отправки нового меню в ЛС или .reply() в группе)
...
async def cq_farm_main_dashboard_from_notify(callback: CallbackQuery, bot: Bot, db: Database, callback_data: FarmCallback):
    ...
    # (Этот callback приходит из ЛС (от уведомлений), .edit_text() здесь безопасен)
    with suppress(TelegramBadRequest):
        await callback.message.edit_text(text, reply_markup=keyboard)
    await callback.answer()

async def get_farm_dashboard_content(user_id: int, db: Database) -> (str, InlineKeyboardMarkup):
    """(Логика) Собирает текст и кнопки для /farm (Добавлена Доска Заказов)."""
...
# (Этот код будет работать и в ЛС, и в группах)
