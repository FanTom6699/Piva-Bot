...
async def get_farm_dashboard(user_id: int, user_name: str, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    if fld_timer and now < fld_timer:
        ...
        text += f"• <b>Поле:</b> Ур. {field_lvl} ({field['plots']} грядок, 🌾 {g_time}м, 🌱 {h_time}м)\n"
    ...
    if fld_timer and now < fld_timer:
        ...
        field_btn_text = "🌾 Моё Поле (СОБРАТЬ!)" if any_ready else "🌾 Моё Поле (Грядки)"
    ...
    # --- ✅ ДОБАВЛЕНА КНОПКА "КАК ИГРАТЬ" (Piva Bot) ---
    kb_buttons = [
        ...
        InlineKeyboardButton(text="❓ Как играть?", callback_data=FarmCallback(action="show_help", owner_id=user_id).pack()) # <-- НОВАЯ
    ]
...
# --- ✅ НОВЫЙ ХЭНДЛЕР "КАК ИГРАТЬ" (Piva Bot) ---
