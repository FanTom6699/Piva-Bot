...
# --- ✅✅✅ НОВЫЙ "ОФОРМЛЕННЫЙ" ДАШБОРД (Piva Bot) ✅✅✅ ---
# --- RENDER: DASHBOARD ---
async def get_farm_dashboard(user_id: int, user_name: str, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    if not field_stats['max_level'] and rating >= field_stats['next_cost']:
        advice = "✨ Совет: У тебя хватает 🍺 на улучшение [🌾 Поля]!"
    elif not brew_stats['max_level'] and rating >= brew_stats['next_cost']:
    ...
    # Кнопка Поля
    if field_stats.get('upgrade_timer_end') and now < field_stats['upgrade_timer_end']:
    ...
    # Кнопка Пивоварни
    if brew_upgrade_timer and now < brew_upgrade_timer:
        # (Статус уже в тексте, кнопка не нужна, но ставим заглушку)
...
# --- ---
# --- ✅✅✅ КОНЕЦ НОВОГО ДАШБОРДА (Piva Bot) ✅✅✅ ---
...
async def cq_farm_upgrades(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    ...
    text += f"• <i>Дает: {field_stats['plots']} Грядок</i>\n"
    ...
    if field_upgrade_timer and now < field_upgrade_timer:
        ...
        if next_field_stats_data.get('plots', 0) > field_stats['plots']:
             bonus_plot = f" (Даст {next_field_stats_data['plots']} грядки)"
