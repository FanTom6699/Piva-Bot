...
# ❌❌❌ Мы УБРАЛИ импорт 'get_shop_menu' отсюда (чтобы не было ошибки)
...
# --- ХЭНДЛЕР МАГАЗИНА (С ИМПОРТОМ ВНУТРИ) ---
...
async def cq_farm_go_to_shop(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    # ✅ Переносим импорт сюда, чтобы избежать 'Circular Import'
...
# --- ✅✅✅ ФУНКЦИЯ УЧАСТКА (С ДЕБАГ-ЛОГАМИ) ✅✅✅ ---
...
async def cq_plot_plant_do(callback: CallbackQuery, callback_data: PlotCallback, db: Database):
    ...
    # --- ДЕБАГ: Проверяем списание семян ---
    ...
    # --- --- --- --- --- --- --- --- --- --- --- --- --- ---
    # Piva Bot: Это комментарий, который ты просил.
    # Если кнопка "зависает" и ничего не происходит, 
    # это 100% значит, что в твоем database.py НЕТ функции 'plant_crop'.
    # Бот падает с ошибкой 'AttributeError', которую ты можешь увидеть в логах Render.
    # --- --- --- --- --- --- --- --- --- --- --- --- --- ---
    ...
    try:
        ...
        # (Это лог, который ты увидишь на Render)
        logging.error(f"!!!!!!!!!!!!!! PIVA BOT ДЕБАГ !!!!!!!!!!!!!!")
        logging.error(f"ОШИБКА: 'AttributeError: 'Database' object has no attribute 'plant_crop''")
        logging.error(f"Это значит, что твой файл database.py не содержит функцию 'plant_crop'.")
        logging.error(f"Скопируй database.py из следующего сообщения ПОЛНОСТЬЮ.")
        logging.error(f"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        
        await callback.answer("⛔ КРИТИЧЕСКАЯ ОШИБКА! (AttributeError)", show_alert=True)
        # (Возвращаем семя, т.к. посадка не удалась)
    ...
    # (Код ниже выполнится, только если 'planted' было получено)
    if not planted:
        # (Эта ошибка не 'AttributeError', а 'UNIQUE constraint' - участок уже занят)
...
# --- (Остальные функции Пивоварни и Улучшений БЕЗ ИЗМЕНЕНИЙ) ---
