...
# ✅ ИСПРАВЛЕНИЕ 2: Импортируем МЕНЮ Магазина (а не текст)
...
async def get_farm_dashboard(user_id: int, user_name: str, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    # --- ✅ ИСПРАВЛЕНИЕ 1: СТАТУС ПОЛЯ (КНОПКА) ---
    ...
    if field_upgrade_timer and now < field_upgrade_timer:
        ...
        # (Проверяем, есть ли готовый урожай, чтобы показать "!")
    ...
    # --- СТАТУС ПИВОВАРНИ (Без изменений) ---
...
# --- ✅ НОВЫЙ ХЭНДЛЕР "ПОКАЗАТЬ УЧАСТКИ" ---
...
async def cq_farm_go_to_shop(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    """
    (ИСПРАВЛЕННАЯ ВЕРСИЯ)
    Вместо alert или нового сообщения, РЕДАКТИРУЕТ текущее меню в Магазин.
    """
    if not await check_owner(callback, callback_data.owner_id): 
        return

    # (Вызываем новую функцию из shop.py)
    ...
    with suppress(TelegramBadRequest):
        # (Редактируем сообщение, а не отвечаем)
        await callback.message.edit_text(text, reply_markup=keyboard)
...
async def cq_plot_plant_do(callback: CallbackQuery, callback_data: PlotCallback, db: Database):
    ...
    # --- ✅ Обновляем "Участки" (а не Главное Меню) ---
...
async def cq_brewery_do(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    # (Делаем проверку здесь, до списания)
    ...
    # (Если вдруг что-то пошло не так, возвращаем ресурсы)
