...
from utils import format_time_delta # Убедись, что utils.py есть и в нем format_time_delta
from .shop import SHOP_NAMES, SHOP_PRICES # Импортируем из /shop для "Быстрого доступа"

# --- ИНИЦИАЛИЗАЦИЯ ---
farm_router = Router()

# --- СЛОВАРЬ "ПОНЯТНЫХ" НАЗВАНИЙ (для /farm) ---
FARM_ITEM_NAMES = {
    'зерно': "🌾 Зерно (Урожай)",
    'хмель': "🌱 Хмель (Урожай)",
...
# --- СЛОВАРЬ "ПОСАДКИ" (Что во что превращается) ---
PLANT_IO = {
    # 'семя_чего': ('урожай_чего', время_роста_в_минутах)
    'семя_зерна': ('зерно', 10),
    'семя_хмеля': ('хмель', 20),
}

# --- СЛОВАРЬ "РЕЦЕПТ ПИВОВАРНИ" (Базовый, Ур. 1) ---
BREWERY_RECIPE = {
    # 'ресурс': количество
    'зерно': 3,
    'хмель': 2,
...
# --- ✅✅✅ ПРАЙС-ЛИСТ УЛУЧШЕНИЙ (НАШ ПЛАН 10 УРОВНЕЙ) ✅✅✅ ---
# (Мы будем хранить здесь "абсолютные" значения, а не "прирост")

# 1. ПОЛЕ (Участки / Шанс x2)
FIELD_UPGRADES = {
    # Уровень: (Цена 🍺, Время ⏳ (часы), Кол-во Участков, Шанс x2 (0-100))
    1: (0, 0, 2, 0),
    2: (5000, 1, 2, 5),
    3: (15000, 2, 3, 5),
    4: (30000, 4, 3, 10),
...
# 2. ПИВОВАРНЯ (Награда 🍺 / Скорость ⏳)
BREWERY_UPGRADES = {
    # Уровень: (Цена 🍺, Время ⏳ (часы), Награда 🍺, Время Варки (минуты/порция))
    1: (0, 0, 35, 15),
    2: (8000, 1, 40, 14),
    3: (20000, 2, 45, 13),
    4: (50000, 4, 50, 12),
...
# --- Вспомогательная функция (Получить данные для Уровня) ---
def get_level_data(level: int, upgrade_table: dict) -> Dict[str, Any]:
    """Возвращает словарь с бонусами для текущего уровня."""
    if level > max(upgrade_table.keys()):
        level = max(upgrade_table.keys()) # Защита от "Ур. 11"
...
# --- ✅✅✅ CALLBACKDATA ФАБРИКИ (Для ВСЕХ кнопок Фермы) ✅✅✅ ---

class FarmCallback(CallbackData, prefix="farm"):
    """Навигация по /farm: Склад, Улучшения, Магазин, Назад"""
    action: str # 'inventory', 'upgrades', 'shop', 'main_dashboard'
    owner_id: int # (Для "Защиты от Чужих")

class PlotCallback(CallbackData, prefix="plot"):
    """Управление Участками: Посадить, Собрать"""
    action: str # 'plant_menu', 'plant_do', 'harvest'
    owner_id: int
    plot_num: int
    crop_id: str = None # (Для 'plant_do')

class BreweryCallback(CallbackData, prefix="brew"):
    """Управление Пивоварней: Варка, Сбор"""
    action: str # 'brew_menu', 'brew_start', 'brew_do', 'collect'
    owner_id: int
    quantity: int = 0 # (Для 'brew_do')

class UpgradeCallback(CallbackData, prefix="upgrade"):
    """Управление Улучшениями: Покупка, Подтверждение"""
    action: str # 'buy_field', 'buy_brewery', 'confirm_field', 'confirm_brewery'
    owner_id: int

# --- ✅✅✅ ГЛАВНАЯ ФУНКЦИЯ: "ДАШБОРД СТАТИСТИКИ" (Твой План) ✅✅✅ ---

async def get_farm_dashboard(user_id: int, user_name: str, db: Database) -> (str, InlineKeyboardMarkup):
    """
    Генерирует "Живой Дашборд Статистики" (/farm)
    (Показывает Уровни, Таймеры, Статус и Кнопки)
    """
    
    farm_data = await db.get_user_farm_data(user_id)
    now = datetime.now()

    # 1. Получаем данные о бонусах с Уровней
    ...
    max_plots = field_stats['plots'] # (Ур. 1 = 2, Ур. 3 = 3...)
    
    # 2. Формируем Текст Статистики (Твой "Дашборд")
    text = f"<b>🌾 Ферма Игрока: {html.escape(user_name)}</b>\n\n"
    text += "<b>СТАТИСТИКА ЗДАНИЙ:</b>\n"
    
    buttons = []
    
    # --- 3. СТАТУС ПОЛЯ (Ур. / "Строится ⏳") ---
    field_upgrade_timer = farm_data.get('field_upgrade_timer_end')
    if field_upgrade_timer and now < field_upgrade_timer:
        # ИДЕТ ПРОКАЧКА ПОЛЯ (48ч)
        ...
        for i in range(1, max_plots + 1):
            if i in active_plots:
                ...
                crop_name = FARM_ITEM_NAMES.get(crop_id, "??").split(' ')[0] # "🌾 Семя Зерна" -> "🌾"
                
                if now >= ready_time:
                    # Готово к сбору
                    btn_text = f"[Участок {i} ({crop_name} СОБРАТЬ)]"
                    btn_callback = PlotCallback(action="harvest", owner_id=user_id, plot_num=i).pack()
                else:
                    # Растет (Таймер 10/20 мин)
                    time_left = format_time_delta(ready_time - now)
                    btn_text = f"[Участок {i} ({crop_name}... {time_left} ⏳)]"
                    btn_callback = "dummy" # Нельзя нажать, пока растет
            else:
                # Пусто
        ...
        # Группируем кнопки Участков по 2 в ряд
        for i in range(0, len(plot_buttons), 2):
            buttons.append(plot_buttons[i:i+2])

    
    # --- 4. СТАТУС ПИВОВАРНИ (Ур. / "Строится ⏳" / "Варка ⏳" / "Забрать 🍺") ---
    ...
    if brewery_upgrade_timer and now < brewery_upgrade_timer:
        ...
        if now >= brewery_batch_timer:
            ...
            btn_callback = "dummy" # Нельзя нажать, пока варится
        
        buttons.append([InlineKeyboardButton(text=btn_text, callback_data=btn_callback)])

    else:
        # ОБЫЧНАЯ РАБОТА ПИВОВАРНИ (Готова)
    ...
    buttons.append([
        InlineKeyboardButton(text="[🏪 Магазин Семян (Быстрый доступ)]", callback_data=FarmCallback(action="shop", owner_id=user_id).pack())
    ])
    
    return text, InlineKeyboardMarkup(inline_keyboard=buttons)


# --- ✅✅✅ ХЭНДЛЕРЫ (Логика Кнопок) ✅✅✅ ---

# --- 0. "Защита от Чужих" (Твой План) ---
async def check_owner(callback: CallbackQuery, owner_id: int) -> bool:
    """
    True = Владелец.
    False = "Чужой" (и мы ему покажем "⛔ Это не твоя ферма!").
    """
...
# --- 3. Навигация: "Быстрый доступ" в /shop ---
...
async def cq_farm_shop_shortcut(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    """
    Эта кнопка просто вызывает "всплывающий" /shop, 
    а "Дашборд" (/farm) НЕ трогает (НЕ редактирует).
    """
    if not await check_owner(callback, callback_data.owner_id): return
    ...
    # Формируем текст для "Быстрого Магазина"
    text = (
        f"<b>🏪 Магазин Семян</b> (Быстрый доступ)\n"
...
# --- 5. Меню: [Участок N (Пусто)] ➔ Посадка ---
...
async def cq_plot_plant_menu(callback: CallbackQuery, callback_data: PlotCallback, db: Database):
    ...
    # Кнопка "Посадить Зерно"
    if inventory['семя_зерна'] > 0:
        time_m = PLANT_IO['семя_зерна'][1]
        buttons.append(InlineKeyboardButton(
            text=f"Посадить 🌾 Зерно ({time_m} мин)", 
            callback_data=PlotCallback(action="plant_do", owner_id=user_id, plot_num=plot_num, crop_id="семя_зерна").pack()
        ))
    
    # Кнопка "Посадить Хмель"
    if inventory['семя_хмеля'] > 0:
        time_m = PLANT_IO['семя_хмеля'][1]
        buttons.append(InlineKeyboardButton(
            text=f"Посадить 🌱 Хмель ({time_m} мин)", 
            callback_data=PlotCallback(action="plant_do", owner_id=user_id, plot_num=plot_num, crop_id="семя_хмеля").pack()
        ))
    
    # Если нет ВООБЩЕ семян (Твой "Гайд")
...
# --- 6. Логика: "Посадить" (Тратим Семя ➔ Ставим Таймер) ---
...
async def cq_plot_plant_do(callback: CallbackQuery, callback_data: PlotCallback, db: Database):
    ...
    user_id = callback.from_user.id
    plot_num = callback_data.plot_num
    crop_id = callback_data.crop_id # 'семя_зерна' или 'семя_хмеля'

    if crop_id not in PLANT_IO:
        await callback.answer("⛔ Ошибка! Неизвестный тип семян.", show_alert=True)
        return
    ...
    if not planted:
        # Этого не должно случиться, но если случилось (кто-то занял участок)
        logging.error(f"Ошибка посадки: участок {plot_num} (user {user_id}) уже был занят.")
        # Возвращаем Семя
        await db.modify_inventory(user_id, crop_id, 1)
        await callback.answer("⛔ Ошибка! Этот участок уже занят. Попробуй обновить /farm.", show_alert=True)
        return

    # 3. Возвращаем на "Дашборд"
...
# --- 7. Логика: [СОБРАТЬ] (Получаем Мешок + Шанс x2) ---
...
async def cq_plot_harvest(callback: CallbackQuery, callback_data: PlotCallback, db: Database):
    ...
    # 1. Проверяем, что там росло (и очищаем участок)
    crop_id_seed = await db.harvest_plot(user_id, plot_num) # 'семя_зерна'
    
    if not crop_id_seed:
        await callback.answer("⛔ Ошибка! Этот участок уже пуст.", show_alert=True)
        return
        
    # 2. Во что оно превращается (Урожай)
    crop_id_product = PLANT_IO[crop_id_seed][0] # 'зерно'
    product_name = FARM_ITEM_NAMES[crop_id_product]
    
    # 3. Проверяем "Шанс x2" (Твой План)
    farm_data = await db.get_user_farm_data(user_id)
    ...
    # 4. Добавляем "Мешок" (Урожай) на Склад
    await db.modify_inventory(user_id, crop_id_product, amount_to_add)

    # 5. Возвращаем на "Дашборд"
    await callback.answer(alert_text, show_alert=True)
    await cq_farm_main_dashboard(callback, FarmCallback(action="main_dashboard", owner_id=user_id), db)


# --- 8. Меню: [🏭 Пивоварня] ➔ "Пакетная Варка" (Инфо) ---
...
# --- 9. Меню: "Пакетная Варка" ➔ Выбор Кол-ва (Твой План) ---
...
async def cq_brewery_start(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    user_id = callback.from_user.id
    max_brew = callback_data.quantity # Мы передали MAX из прошлого шага
    ...
    # Кнопка MAX (если она не 1, 5 или 10)
...
# --- 10. Логика: "Сварить N" (Тратим "Мешки" ➔ Ставим Таймер Варки) ---
...
async def cq_brewery_do(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    if inventory['зерно'] < cost_grain or inventory['хмель'] < cost_hops:
        ...
        # Возвращаем в "Инфо" меню
        await cq_brewery_menu(callback, BreweryCallback(action="brew_menu", owner_id=user_id), db)
        return

    # 2. Тратим "Мешки" (Урожай)
    await db.modify_inventory(user_id, 'зерно', -cost_grain)
    await db.modify_inventory(user_id, 'хмель', -cost_hops)
    
    # 3. Считаем Таймер (2.5ч)
    farm_data = await db.get_user_farm_data(user_id)
    brewery_lvl = farm_data.get('brewery_level', 1)
    brewery_stats = get_level_data(brewery_lvl, BREWERY_UPGRADES)
    brew_time_min = brewery_stats['brew_time_min']
    ...
    # 4. Ставим Таймер "Пакетной Варки"
...
# --- 11. Логика: [ЗАБРАТЬ +XXX 🍺] (Получаем 🍺) ---
...
async def cq_brewery_collect(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    # 1. Проверяем, что там (чтобы знать, сколько начислить)
    farm_data = await db.get_user_farm_data(user_id)
    brewery_lvl = farm_data.get('brewery_level', 1)
    brewery_stats = get_level_data(brewery_lvl, BREWERY_UPGRADES)
...
async def cq_farm_upgrades(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    ...
    if field_upgrade_timer and now < field_upgrade_timer:
        ...
        if balance < cost:
            ...
            btn_callback = "dummy" # Блокируем кнопку
...
# --- 13. Меню: "Подтверждение" Улучшения (Твой План 48ч) ---
...
# --- 14. Логика: "ПОДТВЕРДИТЬ" (Тратим 🍺 ➔ Ставим Таймер 48ч) ---
...
async def cq_upgrade_do(callback: CallbackQuery, callback_data: UpgradeCallback, db: Database):
    ...
    # Проверяем, не строит ли он уже что-то
