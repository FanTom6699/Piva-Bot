...
async def get_farm_dashboard(user_id: int, user_name: str, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    if brew_upgrade_timer and now < brew_upgrade_timer:
        ...
        brewery_status_text = f"<i>(Улучшается... ⏳ {left})</i>"
    elif batch_timer:
        if now >= batch_timer:
    ...
    # (Piva Bot: Проверяем таймеры улучшений, чтобы не советовать, если уже строится)
    ...
    if not field_stats['max_level'] and rating >= field_stats['next_cost'] and can_upgrade_field:
        advice = "✨ Совет: У тебя хватает 🍺 на улучшение [🌾 Поля]!"
    elif not brew_stats['max_level'] and rating >= brew_stats['next_cost'] and can_upgrade_brewery:
    ...
    # Кнопка Пивоварни
    if brew_upgrade_timer and now < brew_upgrade_timer:
        kb.append([InlineKeyboardButton(text=f"🏭 Пивоварня (улучшается...)", callback_data="dummy")])
    elif batch_timer:
        if now >= batch_timer:
            total = brew_stats['reward'] * farm.get('brewery_batch_size', 0)
            kb.append([InlineKeyboardButton(text=f"🏆 Забрать +{total} 🍺", callback_data=BreweryCallback(action="collect", owner_id=user_id).pack())])
        else:
            kb.append([InlineKeyboardButton(text=f"🏭 Пивоварня (варит...)", callback_data="dummy")])
    else:
...
async def get_plots_dashboard(user_id: int, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    text = (
        f"<b>🌱 Поле (Ур. {lvl})</b>\n"
        f"<i>Грядок: {max_plots}, Шанс x2: {stats['chance_x2']}%</i>\n"
...
async def cq_plot_plant_menu(callback: CallbackQuery, callback_data: PlotCallback, db: Database):
    ...
    if inventory['семя_зерна'] > 0:
        time_m = field_stats['grow_time_min']['зерно'] 
    ...
    if inventory['семя_хмеля'] > 0:
        time_m = field_stats['grow_time_min']['хмель'] 
        buttons.append(InlineKeyboardButton(
...
# --- ✅✅✅ НОВОЕ "ИДЕАЛЬНОЕ МЕНЮ УЛУЧШЕНИЙ" (Piva Bot) ✅✅✅ ---
...
async def cq_farm_upgrades(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    ...
    if field_upgrade_timer and now < field_upgrade_timer:
        ...
        text += "📌 Текущие бонусы:\n"
        text += f"• Грядок: {field_stats['plots']}\n"
        text += f"• Шанс x2: {field_stats['chance_x2']}%\n"
        text += f"• Рост: 🌾 {field_stats['grow_time_min']['зерно']}м / 🌱 {field_stats['grow_time_min']['хмель']}м\n"
        
        if field_stats['max_level']:
            text += "\n<b>⭐ Поле — максимальный уровень!</b>\n"
            buttons.append([InlineKeyboardButton(text="✅ Поле (Макс. Уровень)", callback_data="dummy")])
        else:
            ...
            text += f"• Грядок: {field_stats['plots']} → {next_field_stats.get('plots')}\n"
            text += f"• Шанс x2: {field_stats['chance_x2']}% → {next_field_stats.get('chance_x2')}%\n"
            text += f"• Рост (🌾): {field_stats['grow_time_min']['зерно']}м → {next_field_stats.get('grow_time_min', {}).get('зерно')}м\n"
            
            text += f"💰 Стоимость: {cost} 🍺\n"
            text += f"⏳ Время улучшения: {time_h} ч\n"
            
            if balance < cost:
                buttons.append([InlineKeyboardButton(text=f"⛔ Недостаточно 🍺 ({cost})", callback_data="dummy")])
            else:
    ...
    if brewery_upgrade_timer and now < brewery_upgrade_timer:
        ...
        text += "📌 Текущие бонусы:\n"
        text += f"• Награда за варку: +{brewery_stats['reward']} 🍺\n"
        text += f"• Время варки: {brewery_stats['brew_time_min']} мин\n"
        
        if brewery_stats['max_level']:
            text += "\n<b>⭐ Пивоварня — максимальный уровень!</b>\n"
            buttons.append([InlineKeyboardButton(text="✅ Пивоварня (Макс. Уровень)", callback_data="dummy")])
        else:
            ...
            text += f"• Награда: +{brewery_stats['reward']} → +{next_brew_stats.get('reward')} 🍺\n"
            text += f"• Время варки: {brewery_stats['brew_time_min']}м → {next_brew_stats.get('brew_time_min')}м\n"
    ...
    with suppress(TelegramBadRequest):
        await callback.message.edit_text(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
    await callback.answer()
# --- ✅✅✅ КОНЕЦ "ИДЕАЛЬНОГО МЕНЮ" (Piva Bot) ✅✅✅ ---
