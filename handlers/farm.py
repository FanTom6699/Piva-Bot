...
from .farm_config import (
    ...
    SHOP_PRICES,
    CROP_CODE_TO_ID,
    CROP_SHORT # ✅ Импорт для П.9
)

# --- ИНИЦИАЛИЗАЦИЯ ---
farm_router = Router()

# --- ✅✅✅ ПУНКТ 1: ТВОИ ХЕЛПЕРЫ (UI HELPERS) ✅✅✅ ---
...
class PlotCallback(CallbackData, prefix="plot"):
    ...
    owner_id: int
    plot_num: int
    crop_id: Optional[str] = None # ( 'g' / 'h' / None )
...
# --- ✅✅✅ ПУНКТ 2: ТВОЙ НОВЫЙ get_farm_dashboard (...) ✅✅✅ ---
async def get_farm_dashboard(user_id: int, user_name: str, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    if fld_timer and now < fld_timer:
        ...
        text += f"• <b>Поле:</b> Ур. {field_lvl} — {field['plots']} уч., Шанс x2: {field['chance_x2']}%\n"
...
# --- ✅✅✅ ПУНКТ 3: ТВОЙ НОВЫЙ get_plots_dashboard (...) (С ФИКСОМ ИЗ П.9) ✅✅✅ ---
async def get_plots_dashboard(user_id: int, db: Database) -> (str, InlineKeyboardMarkup):
    ...
    text = (
        f"<b>🌱 Поле (Ур. {lvl})</b>\n"
        f"<i>Участков: {max_plots}, Шанс x2 Урожая: {stats['chance_x2']}%</i>\n\n"
    ...
    # Кнопки-участки (3 в ряд если много, 2 — если мало)
    ...
    for i in range(1, max_plots + 1):
        if i in active:
            ...
            # ✅ П.9: Используем CROP_SHORT
            product_id = PLANT_IO.get(seed_id, ['??'])[0]
...
# --- ✅✅✅ ПУНКТ 5: ТВОЙ НОВЫЙ cq_farm_inventory (...) ✅✅✅ ---
...
async def cq_farm_inventory(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    ...
    # Если будет ёмкость склада — сюда же добавим прогресс-бар
    text = (
        f"<b>📦 Мой Склад</b>\n\n"
        f"<b>Урожай:</b>\n"
...
# --- ✅✅✅ ПУНКТ 6: ТВОЯ НОВАЯ cq_brewery_menu (...) ✅✅✅ ---
...
async def cq_brewery_menu(callback: CallbackQuery, callback_data: BreweryCallback, db: Database):
    ...
    if max_brew > 0:
        ...
        for qty in (1, 5, 10):
            if max_brew >= qty:
                total = timedelta(minutes=st['brew_time_min'] * qty)
                btns.append(InlineKeyboardButton(
                    text=f"🔥 {qty} ({format_time_delta(total)})",
                    callback_data=BreweryCallback(action="brew_do", owner_id=uid, quantity=qty).pack()
                ))
        # MAX если нужно
        if max_brew not in (1, 5, 10) and max_brew > 0: # Добавил max_brew > 0
            total = timedelta(minutes=st['brew_time_min'] * max_brew)
            btns.append(InlineKeyboardButton(
...
# --- ❌ Хэндлер cq_brewery_start УДАЛЕН (Больше не нужен) ---
...
# --- ✅✅✅ ПУНКТ 7: УЛУЧШЕНИЯ (Оставляем как есть) ✅✅✅ ---
