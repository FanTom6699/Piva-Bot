...
# ❌❌❌ ИСПРАВЛЕНИЕ: Мы УБРАЛИ импорт 'get_shop_menu' отсюда
...
# --- ✅✅✅ ИСПРАВЛЕННЫЙ ХЭНДЛЕР МАГАЗИНА (С ИМПОРТОМ ВНУТРИ) ---
...
async def cq_farm_go_to_shop(callback: CallbackQuery, callback_data: FarmCallback, db: Database):
    """
    (ИСПРАВЛЕННАЯ ВЕРСИЯ)
    Редактирует текущее меню в Магазин.
    """
    # ✅✅✅ ИСПРАВЛЕНИЕ: Переносим импорт сюда, чтобы избежать 'Circular Import'
    from .shop import get_shop_menu 
